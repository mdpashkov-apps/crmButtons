      ...
      for (const element of buttonActions) {
        switch (element) {
          case "0":
            ...
            if (businessProcessWithoutParameters.length != 0) {
              await launchBP(
          ...
          case "3":
            ...
            break;
          case "4":
            const idFiledLink = this.buttonFields.crmLinkFields_FIELDS;
            const idEntityCRM = window.entityValueId;
            const entityType = this.buttonFields.entitySelection_FIELDS;
            const responseGetLink = await getLinkValueFilds(
              window.memberId,
              idFiledLink,
              idEntityCRM,
              entityType,
