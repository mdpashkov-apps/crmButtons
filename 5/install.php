...
$eventBind = overCRest::call(
    ...
    [
        ...
        'handler' => `https://app.overplan.ru/applications/crmButtons/${$versions}/uninstall.php`
    ]
);
...
function registerReport()
{
    ...
    if ($getChatAndBot == null || count($getChatAndBot["findChat"]) == 0) {
        // if($getChatAndBot == NULL || count($getChatAndBot) == 0) {

        $chatId = overCRest::call("im.chat.add", [
            "TYPE" => "CHAT",
            "TITLE" => CHAT_REPORT,
            "USERS" => [1, 2]
        ])['result'];
    } else {
        $chatId = $getChatAndBot["findChat"][0]["id"];
    }
    ...
    if (!isset($botId)) {
        $botId = overCRest::call('imbot.register', [
            'CODE' => BOT_REPORT_CODE,
            'EVENT_MESSAGE_ADD' => 'https://app.overplan/applications/chatbot_demo/handler.php',
            ...
            'PROPERTIES' => [ // Bot personality (req.)
                'NAME' => BOT_REPORT_NAME,
                // Bot name (NAME or LAST_NAME is required)
                'LAST_NAME' => '',
                // Bot last name
                'COLOR' => 'AQUA',
                // Bot color for mobile Bitrix24 application RED,  GREEN, MINT, LIGHT_BLUE, DARK_BLUE, PURPLE, AQUA, PINK, LIME, BROWN,  AZURE, KHAKI, SAND, MARENGO, GRAY, GRAPHITE
                'EMAIL' => 'no@example.com',
                'PERSONAL_BIRTHDAY' => '2020-07-18',
                // format YYYY-mm-dd
                'WORK_POSITION' => 'Report on affairs',
                // Bot 'job-title' as a bot description
                'PERSONAL_WWW' => '',
                'PERSONAL_GENDER' => 'M',
                // Bot gender
                        ...
                        (result) => {
                            if (!result.answer.error) {
                                ...
                                BX24.callBatch({
                                    ...
                                    crmLinkFieldsValue: ['entity.item.property.add', {
                                        'ENTITY': 'customButton',
                                        'PROPERTY': 'crmLinkFields_FIELDS',
                                        'NAME': 'Поля ссылка',
                                        'TYPE': 'S'
                                    }],
