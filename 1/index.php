        ...
        <div class="content" v-if="current_button && current_button.button_actions">
            <div class="settings_fields">
                ...
                <div class="div_row">
                    ...
                    <input max="20" min="0" v-model="current_button.radius_btn" @change="current_button.radius_btn = validator(0, 20,current_button.radius_btn )" type="number" id="radius_btn">
                </div>
                <div class="div_row">
                ...
                <div v-if="current_button.buttonBorderSelection" class="div_row">
                    ...
                    <input v-model="current_button.buttonBorderWidth" max="5" min="0" type="number" id="buttonBorderWidth" @change="current_button.buttonBorderWidth = validator(0, 5,current_button.buttonBorderWidth)">
                </div>
                <div v-if="current_button.buttonBorderSelection" class="div_row">
                ...
                <div class="div_row">
                    ...
                    <multiselect v-model="current_button.array_entities_value" name="entity_selection" placeholder="Выберите сущность" label="name" track-by="value" deselect-label="" select-label="Выбрать" selected-label="" open-direction="bottom" :options="current_button.array_entities" :multiple="false" :taggable="false" :close-on-select="true" :limit="1" @remove="removeAnEntity" @select="changingAnEntity">
                        <span slot="noResult">
                            Такого варианта нет
                        </span>
                    </multiselect>
                ...
                <button class="accordion" @click="accordion(0)">
                    Запустить БП
                </button>
                <div class="panel" :class="{panel_show:accordion_0}">
                    <div class="div_row">
                        <label for="activate_the_property_0">Активировать свойство:</label>
                        <div>
                            <input v-model="current_button.button_actions.id" type="checkbox" id="activate_the_property_0" value="0">
                        </div>
                    </div>
                    <div class="div_row">
                ...
                <button class="accordion" @click="accordion(1)">
                    Cоздание документа
                </button>
                <div class="panel" :class="{panel_show:accordion_1}">
                    <div class="div_row">
                        <label for="activate_the_property_1">Активировать свойство:</label>
                        <div>
                            <input v-model="current_button.button_actions.id" type="checkbox" id="activate_the_property_1" value="1">
                        </div>
                    </div>
                    <div class="div_row">
                        <label for="selection_document">Выберите шаблон документ:</label>
                        <multiselect v-model="current_button.button_actions.document_templates.value" name="selection_document" placeholder="Выберите шаблон" label="name" track-by="value" deselect-label="Убрать" select-label="Выбрать" selected-label="" open-direction="bottom" :options="current_button.button_actions.document_templates.options" :multiple="true" :taggable="false" :close-on-select="false" :limit="1">
                            <span slot="noResult">
                                Такого варианта нет
                            </span>
                        </multiselect>
                ...
                <button class="accordion" @click="accordion(2)">
                    Создать элемент Списка
                </button>
                <div class="panel" :class="{panel_show:accordion_2}">
                    <div class="div_row">
                        <label for="activate_the_property_2">Активировать свойство:</label>
                        <div>
                            <input v-model="current_button.button_actions.id" type="checkbox" id="activate_the_property_2" value="2">
                        </div>
                    </div>
                    <div class="div_row">
                        <label for="selection_list">Выберите список:</label>
                        <multiselect v-model="current_button.button_actions.lists.value" name="selection_list" placeholder="Выберите список" label="name" track-by="value" deselect-label="Убрать" select-label="Выбрать" selected-label="" open-direction="bottom" :options="current_button.button_actions.lists.options" :multiple="false" :taggable="false" :close-on-select="true" :limit="1" @remove="listRemove" @select="listSelection">
                            <span slot="noResult">
                                Такого варианта нет
                            </span>
                        </multiselect>
                    ...
                    <table v-if="current_button.button_actions.fields_table.length !== 0">
                        ...
                        <tr v-for="row in current_button.button_actions.fields_table">
                            <td>{{row.fieldsLists.name}}<span v-if="row.fieldsLists.isRequired == 'Y'" style="color:red;">*</span></td>
                            <td>
                                <multiselect v-model="row.fieldsEntiyValue" placeholder="Выберите поле" label="name" track-by="value" deselect-label="Убрать" select-label="Выбрать" selected-label="" open-direction="bottom" :options="current_button.button_actions.optionsEntity" :multiple="false" :taggable="false" :close-on-select="true" :limit="1">
                                    <span slot="noResult">
                                        Такого варианта нет
                                    </span>
                                </multiselect>
                ...
                <button class="accordion" @click="accordion(3)">
                    Перейти по произвольной ссылке
                </button>
                <div class="panel" :class="{panel_show:accordion_3}">
                    <div class="div_row">
                        <label for="activate_the_property_3">Активировать свойство:</label>
                        <div>
                            <input v-model="current_button.button_actions.id" type="checkbox" id="activate_the_property_3" value="3">
                        </div>
                    </div>
                    <div class="div_row">
                ...
                <div class="div_btn">
                    ...
                    <button @click="saving_settings('<? echo (string)$_GET['DOMAIN']; ?>')">
                        Сохранить настройки
                    </button>
                    <button v-if="!current_button.button_actions.button_in_CRM" @click="addBtn(current_button.id, '<? echo (string)$_GET['DOMAIN']; ?>')">
                        Создать кнопку в карточках CRM
                    </button>
                    <button v-else class="deleteBtnCrm" @click="addBtn(current_button.id, '<? echo (string)$_GET['DOMAIN']; ?>')">
            ...
            <modal v-if="showModal">
                ...
                <div slot="footer" class="modal_btn">
                    <button @click="deleteBtn('<? echo (string)$_GET['DOMAIN']; ?>'), showModal = false">Да</button>
