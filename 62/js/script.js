import {
  ...
  deleteButtonInCRM,
  deleteRow,
  getBiznprocesByEntity,
  getDocumentTemplateByEntity,
  getListByEnity,
  getNewRowData,
  getRowsData,
  saveButton,
  updateEntityData,
  updateListFields,
  getCrmLinkFiledsByEntity,
} from "../applicationHandlers/index.js";
...
var app = new Vue({
  ...
  data() {
    return {
      ...
      disableBizproc: false,
      flagsDocumentTemplate: false,
      disableDocumentTemplate: false,
      flagsList: false,
      disableList: false,
      flagsCrmLinkFields: false,
      disableCrmLinkFields: false,
      flagAttributeLink: false,
    };
  },
  computed: {
    calculateBooksMessage() {
      return this.current_button.buttonBorderSelection == true
        ? "solid"
        : "none";
    },
  },
  methods: {
    async selectedBusinessProcessEventClick(elem) {
      if (!this.current_button.array_entities_value.value) {
        alert("Выберите сущность");
        return;
      }
      this.disableBizproc = true;
      this.accordion_0 = true;
      if (!this.flagsButtonBizproc) {
        const responseGetBizproc = await getBiznprocesByEntity(
          window.memberId,
          this.current_button.array_entities_value.value,
        );
        this.current_button.button_actions.business_processes.options =
          responseGetBizproc.data.options;
        if (
          responseGetBizproc.data.selected !== "[]" &&
          responseGetBizproc.data.selected !== "" &&
          responseGetBizproc.data.selected?.length !== 0
        ) {
          this.current_button.button_actions.business_processes.value =
            responseGetBizproc.data.selected;
        } else {
          this.current_button.button_actions.business_processes.value = [];
        }
      ...
      this.disableBizproc = false;
    },
    selectedArbitraryLink() {
      if (!this.current_button.array_entities_value.value) {
        alert("Выберите сущность");
        return;
      }
      if (!this.flagAttributeLink) {
        this.flagAttributeLink = true;
        this.accordion_3 = true;
      } else {
        this.accordion_3 = false;
        this.flagAttributeLink = false;
      }
    },
    async selectedDocumentTemplateEventClick(elem) {
      if (!this.current_button.array_entities_value.value) {
        alert("Выберите сущность");
        return;
      }
      this.disableDocumentTemplate = true;
      if (!this.flagsDocumentTemplate) {
        const responseGetDocumentTemplate = await getDocumentTemplateByEntity(
          window.memberId,
          this.current_button.array_entities_value.value,
        );
        ...
        this.current_button.button_actions.document_templates.options =
          responseGetDocumentTemplate.data.options;
        if (responseGetDocumentTemplate.data?.value[0]?.value !== null) {
          this.current_button.button_actions.document_templates.value =
            responseGetDocumentTemplate.data.value;
        } else {
          this.current_button.button_actions.document_templates.value = [];
        }
        this.flagsDocumentTemplate = true;
        this.accordion_1 = true;
      } else {
        this.flagsDocumentTemplate = false;
        this.accordion_1 = false;
      }
      this.disableDocumentTemplate = false;
    },
    async selectedListEventClick(elem) {
      if (!this.current_button.array_entities_value.value) {
        alert("Выберите сущность");
        return;
      }
      ...
      if (!this.flagsList) {
        const responseGetList = await getListByEnity(
          window.memberId,
          this.current_button.array_entities_value.value,
        );
        this.current_button.button_actions.lists.options =
          responseGetList.data.options;
        this.current_button.button_actions.lists.value =
          responseGetList.data.value === "[]" ? [] : responseGetList.data.value;
        this.flagsList = true;
    ...
    async selectedCrmFieldsLinkEventClick(elem) {
      if (!this.current_button.array_entities_value.value) {
        alert("Выберите сущность");
        return;
      }
      this.disableCrmLinkFields = true;
      if (!this.flagsCrmLinkFields) {
        const responseCrmLinkFields = await getCrmLinkFiledsByEntity(
          window.memberId,
          this.current_button.array_entities_value.value,
        );
        this.current_button.button_actions.crmLinkFilds = {
          options: [],
          value: [],
        };
        this.current_button.button_actions.crmLinkFilds.options =
          responseCrmLinkFields.data.options;
        if (responseCrmLinkFields.data?.selected[0]?.value !== null) {
          this.current_button.button_actions.crmLinkFilds.value =
            responseCrmLinkFields.data.selected;
        } else {
          this.current_button.button_actions.crmLinkFilds.value = [];
        }
        this.flagsCrmLinkFields = true;
        this.accordion_4 = true;
      } else {
        this.flagsCrmLinkFields = false;
        this.accordion_4 = false;
      }
      this.disableCrmLinkFields = false;
    },
    ...
    open_lists_btn(elem) {
 // сброс всех аккордеонов и флагов
  this.accordion_0 = false;
  this.accordion_1 = false;
  this.accordion_2 = false;
  this.accordion_3 = false;
  ...
  this.flagAttributeLink = false;
      ...
      this.selected_button = Object.assign(
    ...
    async saving_settings(domen) {
      ...
      //         if (
      //           element.fieldsLists.isRequired == "Y" &&
      //           element.fieldsEntiyValue == null
      //         ) {
      //           paddingError = true;
      //           paddingErrorText +=
      //             "Не заполнено обязательное поле списка:" +
      //             element.fieldsLists.name +
      //             " \n";
      //         }
      //         if (element.fieldsEntiyValue != null) {
      //           if (
      //             (element.fieldsLists.list == false &&
      //               element.fieldsEntiyValue.list == true) ||
      //             (element.fieldsLists.list == true &&
      //               element.fieldsEntiyValue.list == false)
      //           ) {
      //             paddingError = true;
      //             paddingErrorText +=
      //               "Не верное сопостовление типов полей, одно поле не является списком:" +
      //               element.fieldsLists.name +
      //               " \n";
      //           }
      //         }
    ...
    async changingAnEntity(event) {
      this.loader = true;
      this.current_button.button_actions.id = [];
      if (this.flagsButtonBizproc === true) {
        this.flagsButtonBizproc = !this.flagsButtonBizproc;
        this.accordion_0 = false;
      }
      if (this.flagsCrmLinkFields === true) {
        this.flagsCrmLinkFields = !this.flagsCrmLinkFields;
      }
      if (this.flagsDocumentTemplate === true) {
        this.flagsDocumentTemplate = !this.flagsDocumentTemplate;
      }
      if (this.flagsList === true) {
    ...
    async listSelection(event) {
      this.loader = true;
      let fieldslistNew = await updateListFields(
        event.value,
        this.current_button.array_entities_value.value,
        window.memberId,
      );
      this.current_button.button_actions.fields_table =
        fieldslistNew.newData.fields_table;
      this.current_button.button_actions.optionsEntity =
        fieldslistNew.newData.optionsEntity;
      this.loader = false;
    },
    listRemove(event) {
      this.current_button.button_actions.fields_table = [];
