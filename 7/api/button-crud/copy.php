...
if (is_string($button['entitySelection_FIELDS'])) {
    ...
    // 1. Стандартные сущности
    if (isset($entityMap[$rawValue])) {
        $button['entitySelection_FIELDS'] = json_encode([
            'value' => ctype_digit($rawValue) ? (int)$rawValue : $rawValue,
...
if ($needLoadDocName && !empty($button['entitySelection_FIELDS'])) {
    ...
    // соответствие сущностей CRM → entityTypeId
    $entityMap = [
        'Lead'    => 1,
        'Deal'    => 2,
        'Contact' => 3,
    ...
    // собираем entityTypeId (учёт категорий сделок)
    $entityTypeIds = [];
        ...
        for ($i = 0; $i < $pages; $i++) {
            ...
            foreach ($page['result']['templates'] ?? [] as $tpl) {
                ...
                if ((int)$tpl['id'] === $templateId) {
                    ...
                    break 3; // нашли → выходим из всех циклов
                }
            }
        }
    }
